...
class WalletMixin:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # /withdraw — send USDC to an external Arbitrum address
    # ──────────────────────────────────────────────────────────────────────

    async def cmd_withdraw(self, chat_id: int, arg: Optional[str] = None):
        """Start a USDC withdrawal: consolidate → ask for address → confirm → send."""
        if not arg:
            await self.send_message(chat_id, "Usage: /withdraw <amount>\nExample: /withdraw 100")
            return

        # Parse amount
        try:
            amount = float(arg.replace("$", "").replace(",", "").strip())
        except ValueError:
            await self.send_message(chat_id, "Invalid amount. Usage: /withdraw <amount>")
            return
        ...
        await self.send_message(
            chat_id,
            f"{consolidation_info}"
            f"W1 balance: ${w1_balance:,.2f}\n\n"
            f"Send the Arbitrum wallet address to withdraw **${amount:,.2f} USDC** to:"
        )

    async def handle_withdraw_reply(self, chat_id: int, text: str):
        """Handle address input and confirmation for pending withdrawal."""
        ...
        if time.time() - pending["created_at"] > 120:
            ...
            await self.send_message(chat_id, "Withdrawal expired (2 min). Use /withdraw again.")
            return
        ...
        if state == "awaiting_address":
            ...
            if not text.startswith("0x") or len(text) != 42:
                await self.send_message(chat_id, "Invalid address. Must be a 42-character Arbitrum address starting with 0x.")
                return

            try:
                addr = Web3.to_checksum_address(text)
            except Exception:
                await self.send_message(chat_id, "Invalid address. Please send a valid Arbitrum wallet address.")
                return

            # Check not zero address
            if addr == "0x" + "0" * 40:
                await self.send_message(chat_id, "Cannot withdraw to zero address.")
                return

            # Check not one of our own wallets
            own_addrs = {acct.address.lower() for _, acct in self._all_wallets()}
            if addr.lower() in own_addrs:
                await self.send_message(chat_id, "Cannot withdraw to a bot wallet. Send an external address.")
                return

            # Store address and move to confirmation
            pending["address"] = addr
            pending["state"] = "awaiting_confirm"
            pending["created_at"] = time.time()  # Reset timer for confirmation
        ...
        if state == "awaiting_confirm":
            ...
            if upper in ("YES", "Y", "CONFIRM"):
                await self._execute_withdraw(chat_id, pending)
            elif upper in ("NO", "N", "CANCEL"):
                del self.pending_withdraw[chat_id]
                await self.send_message(chat_id, "Withdrawal cancelled.")
            else:
                await self.send_message(chat_id, "Send /confirm to execute or /cancel to abort.")

    async def _execute_withdraw(self, chat_id: int, pending: dict):
        """Execute the USDC transfer for a confirmed withdrawal."""
        amount = pending["amount"]
        to_addr = pending["address"]
        del self.pending_withdraw[chat_id]

        await self.send_message(chat_id, f"Sending ${amount:,.2f} USDC to {to_addr[:10]}...{to_addr[-6:]}...")
        ...
        try:
            ...
            tx_hash = await asyncio.to_thread(
                self._send_tx, self.cfg.collateral_token, transfer_data, 0, self.account
            )

            if tx_hash.startswith("dry_run"):
                await self.send_message(
                    chat_id,
                    f"[DRY RUN] Would send ${amount:,.2f} USDC to {to_addr}\nTX: {tx_hash}"
                )
                return
            ...
            if receipt.get("status") == 1:
                ...
                try:
                    remaining = await asyncio.to_thread(self._get_portfolio_value_for, self.account)
                    bal_line = f"\nW1 remaining: ${remaining:,.2f}"
                except Exception:
                    bal_line = ""

                arb_url = f"https://arbiscan.io/tx/{tx_hash}"
                await self.send_message(
                    chat_id,
                ...
                await self.send_message(
                    chat_id,
                    f"**Withdrawal Failed** — transaction reverted.\n"
